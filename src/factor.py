...
def neutralize_markettype_cs(
    ...
    min_obs_buffer: int = 5,   # 回归自由度额外缓冲
) -> pd.Series:
    """
    横截面中性化：mtype(市场类型) + 可选 log(mktcap)
    返回：残差（与 f 同 index），不可用位置为 NaN
    """
    ...
    # y 必须有横截面变化，否则残差没有意义（也会导致后面 IC constant warning）
    ...
    # 如果没有 mktcap：仅 mtype 回归
    if mktcap is None:
        d = pd.get_dummies(df["mtype"], drop_first=True, dtype=float)
        X_parts = [np.ones((len(df), 1), dtype=float)]
        if d.shape[1] > 0:
            X_parts.append(d.to_numpy())

        X = np.concatenate(X_parts, axis=1)
        p = X.shape[1]
        if len(df) < p + min_obs_buffer:
            return f * np.nan
    ...
    # 3) 有 mktcap：要求 cap>0 且有限
    cap = pd.to_numeric(df["mktcap"], errors="coerce")
    df2 = df.loc[cap.notna() & np.isfinite(cap.to_numpy()) & (cap > 0)].copy()
    if df2.empty:
        return f * np.nan

    # y2 也必须有变化
    if df2["f"].nunique(dropna=True) <= 1:
        return f * np.nan
...
def clean_factor_with_markettype(
    ...
    """
    - 先按 universe 过滤因子
    - 再按 markettype（可选 size）做横截面中性化
    修复点：
    1) 统一对齐 factor/universe/markettype/mktcap 的 index/columns
    2) 避免 markettype 缺少某些日期时 markettype.loc[t] 直接 KeyError
    """
    ...
    # universe 对齐到 factor
    universe = universe.reindex(index=factor.index, columns=factor.columns)

    # markettype 对齐到 factor（缺失的日期/股票会变 NaN，而不是 KeyError）
    markettype = markettype.reindex(index=factor.index, columns=factor.columns)

    # mktcap 对齐到 factor
    if mktcap is not None:
        mktcap = mktcap.reindex(index=factor.index, columns=factor.columns)

    # === 1) universe过滤 ===
    f = factor.where(universe)

    # === 1.5) 只保留 markettype 有有效值的日期（避免整行缺失导致中性化无意义/报错） ===
    valid_dates = markettype.notna().any(axis=1)
    f = f.loc[valid_dates]
    markettype = markettype.loc[valid_dates]
    if mktcap is not None:
        mktcap = mktcap.loc[valid_dates]
