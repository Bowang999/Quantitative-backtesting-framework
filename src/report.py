...
def run_factor_backtest_report(
    ...
    portfolio_mode: str = "factor",  # "factor" | "weights"
    ...
    # ===================== weights 模式参数 =====================
    weights_long_path: str | None = None,   # long-form: date, code, weight
    weights_mat_path: str | None = None,    # wide-form: index=date columns=code values=weight（二选一）
    weights_date_col: str = "date",
    weights_code_col: str = "code",
    weights_weight_col: str = "weight",
    weights_unit_auto: bool = True,        
    weights_ffill: bool = True,            
    weights_w_lag: int = 1,                 
    weights_renormalize: bool = True,       
    close_adj_path: str = "../data/cache/close_adj.parquet",  

    # ----- weights 模式对比真实指数 -----
    real_index_close_path: str | None = None,   
    real_index_date_col: str = "date",
    real_index_close_col: str = "close",
    plot_real_index_compare: bool = True,
):
    # ===================== 指标函数 =====================
    def ann_return_linear(r: pd.Series, days: int) -> float:
        r = r.dropna()
        return np.nan if len(r) == 0 else float(r.mean() * days)

    def ann_vol_from_daily(r: pd.Series, days: int) -> float:
        r = r.dropna()
        return np.nan if len(r) == 0 else float(r.std(ddof=1) * np.sqrt(days))
    ...
    def apply_weight_fixed_gross_no_leverage(
        ...
        # r 为 NaN 的资产权重置 0（避免把 NaN 当 0 影响 gross）
    ...
    if annual_days is None:
        # 你原代码里 weekly_nonoverlap_vwap 用 52，其它 252；这里保持一致
        if rebalance == "weekly" and weekly_use_nonoverlap_vwap:
            annual_days = 52
        else:
            annual_days = 252
    ...
    #                                   MODE = WEIGHTS
    # ======================================================================================
    if portfolio_mode == "weights":
        if (weights_long_path is None) and (weights_mat_path is None):
            raise ValueError("portfolio_mode='weights' requires weights_long_path or weights_mat_path.")

        # 1) 读 close_adj -> ret_adj
        close_adj = pd.read_parquet(close_adj_path)
        close_adj.index = pd.to_datetime(close_adj.index)
        close_adj = close_adj.loc[start_date:end_date].sort_index()
        close_adj.columns = [str(c).split(".")[0].zfill(6) for c in close_adj.columns]

        px = close_adj.ffill()
        ret_adj = px.pct_change().replace([np.inf, -np.inf], np.nan).fillna(0.0)

        # 2) 读权重 -> 矩阵
        if weights_mat_path is not None:
            w_mat = pd.read_parquet(weights_mat_path)
            w_mat.index = pd.to_datetime(w_mat.index)
            w_mat = w_mat.loc[start_date:end_date].sort_index()
            w_mat.columns = [str(c).split(".")[0].zfill(6) for c in w_mat.columns]
        else:
            wlong = pd.read_parquet(weights_long_path)
            wlong[weights_date_col] = pd.to_datetime(wlong[weights_date_col])
            wlong = wlong[(wlong[weights_date_col] >= start_date) & (wlong[weights_date_col] <= end_date)].copy()
            wlong[weights_code_col] = (
                wlong[weights_code_col].astype(str).str.split(".").str[0].str.zfill(6)
            )
            if weights_unit_auto and wlong[weights_weight_col].max() > 1.5:
                wlong[weights_weight_col] = wlong[weights_weight_col] / 100.0

            w_mat = (
                wlong.pivot(index=weights_date_col, columns=weights_code_col, values=weights_weight_col)
                .fillna(0.0)
                .sort_index()
            )

        # 3) 对齐股票池
        common_cols = w_mat.columns.intersection(ret_adj.columns)
        w_mat = w_mat[common_cols]
        ret_use = ret_adj[common_cols]

        # 4) 权重扩展到每日（持有直到下次更新）
        if weights_ffill:
            w_daily = w_mat.reindex(ret_use.index).ffill().bfill()
        else:
            # 不扩展就只在权重日有仓位，其他日为0（一般不符合指数复刻）
            w_daily = w_mat.reindex(ret_use.index).fillna(0.0)

        # 5) 用昨日权重乘今日收益（避免未来函数）
        w_use = w_daily.shift(weights_w_lag).fillna(0.0)
        if weights_renormalize:
            s = w_use.sum(axis=1).replace(0.0, np.nan)
            w_use = w_use.div(s, axis=0).fillna(0.0)

        port_ret = (w_use * ret_use).sum(axis=1)
        nav_sim = (1 + port_ret).cumprod()
        if len(nav_sim) > 0:
            nav_sim = nav_sim / nav_sim.iloc[0]

        # 6) 真实指数净值（可选）
        nav_real = None
        if real_index_close_path is not None:
            real_df = pd.read_parquet(real_index_close_path)
            real_df[real_index_date_col] = pd.to_datetime(real_df[real_index_date_col])
            nav_real = (
                real_df.set_index(real_index_date_col)[real_index_close_col]
        ...
        # 7) 误差与绘图
        if nav_real is not None and plot_real_index_compare:
            ix = nav_sim.index.intersection(nav_real.index)
            diff = nav_sim.loc[ix] - nav_real.loc[ix]
            mae = float(diff.abs().mean()) if len(ix) else np.nan
            mx = float(diff.abs().max()) if len(ix) else np.nan
            corr = float(nav_sim.loc[ix].corr(nav_real.loc[ix])) if len(ix) else np.nan
            print(f"[WEIGHTS MODE] Sim vs Real | MAE={mae:.6f} | MaxAbsDiff={mx:.6f} | Corr={corr:.6f} | n={len(ix)}")

            plt.figure(figsize=(10, 5), dpi=120)
            plt.plot(ix.to_numpy(), nav_sim.loc[ix].to_numpy(), label="Backtest")
            plt.plot(ix.to_numpy(), nav_real.loc[ix].to_numpy(), "--", label="Real")
            plt.title(title)
            plt.grid(alpha=0.3)
            plt.legend()
            plt.tight_layout()
            ...
            plt.figure(figsize=(10, 5), dpi=120)
            plt.plot(nav_sim.index.to_numpy(), nav_sim.to_numpy(), label="Backtest")
            plt.title(title)
            plt.grid(alpha=0.3)
            plt.legend()
            plt.tight_layout()
        ...
        # 简单指标
        ann = ann_return_linear(port_ret, annual_days)
        vol = ann_vol_from_daily(port_ret, annual_days)
        sr = sharpe_from_daily(port_ret, annual_days, rf=0.0)
        mdd = max_drawdown_from_nav(nav_sim)

        print("[WEIGHTS MODE] Long-only portfolio")
    ...
    #                                   MODE = FACTOR
    # ======================================================================================
    if portfolio_mode != "factor":
        raise ValueError("portfolio_mode must be 'factor' or 'weights'")
    ...
    universe = universe.reindex(ret.index).fillna(False).astype(bool)
    ...
    if benchmark == "mkt_ew":
        ...
        # weekly_nonoverlap_vwap：把日收益复利成周收益记在 exit 日（你原逻辑）
        if rebalance == "weekly" and weekly_use_nonoverlap_vwap:
            rb = pd.to_datetime(
    ...
    print(f"[FACTOR-LONG] EXCESS vs {bench_tag if excess_vs_benchmark else 'RAW'} | price={price_tag} | rebalance={rebalance}({extra_tag}) | Long=G{long_group}")
    ...
    print(f"[FACTOR-LS] price={price_tag} | rebalance={rebalance}({extra_tag}) | LS=G{long_group}-G{short_group}")
    print(f"Annual Return:     {ann:.2%}")
    print(f"Annual Volatility: {vol:.2%}")
    print(f"Sharpe Ratio:      {sr:.2f}")
    ...
    print(f"[FACTOR-LS-50] price={price_tag} | rebalance={rebalance}({extra_tag}) | LS=Top50%-Bottom50% (by STOCKS)")
    ...
    axes[0].plot(long_excess_nav.index.to_numpy(), long_excess_nav.to_numpy(), lw=2.0, label=f"LONG(G{long_group}) excess")
    axes[0].plot(ls_excess_nav.index.to_numpy(), ls_excess_nav.to_numpy(), lw=2.2, label=f"LS(G{long_group}-G{short_group})")
    axes[0].plot(ls50_excess_nav.index.to_numpy(), ls50_excess_nav.to_numpy(), lw=2.2, label=f"LS(Top50%-Bottom50%)")

    if bench_nav is not None:
        axes[0].plot(bench_nav.index.to_numpy(), bench_nav.to_numpy(), lw=1.2, alpha=0.7, label=f"BENCH NAV ({bench_tag})")
    else:
        axes[0].plot(mkt_nav.index.to_numpy(), mkt_nav.to_numpy(), lw=1.0, alpha=0.5, label="MKT_EW NAV (ref)")
    ...
    for col in grp_nav.columns:
        axes[1].plot(grp_nav.index.to_numpy(), grp_nav[col].to_numpy(), lw=1.2, alpha=0.85, label=col)
